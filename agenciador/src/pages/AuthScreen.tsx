  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    const result =
      mode === 'login'
        ...
        : await signUp(email, password, nome, 'agenciador')
    ...
    <div className="min-h-dvh flex flex-col justify-center px-6 py-10 bg-white">
      <div className="mx-auto w-full max-w-[380px]">
        ...
        <h1 className="rbr-display text-2xl font-bold text-[color:var(--rbr-navy-dark)] mb-1">
          {mode === 'login' ? 'Entrar' : 'Criar conta de agenciador'}
        </h1>
        <p className="text-sm text-[color:var(--rbr-muted)] mb-6">
          {mode === 'login'
            ? 'Acesse seu painel de parceiro RBR Cargo'
            : 'Leva menos de um minuto. Complete seus dados depois no Perfil.'}
        </p>
